...
class Options_Machine {
	 ...
	 *
	 * Notes:
	 * 	- For further uses, you can check for the $value['type'] and performs more specific sanitation on the option
	*	- The ultimate objective of this function is to prevent the "undefined index" errors some authors are having due to malformed options array
	 */
		...
		foreach($options as $value) {
			...
			switch($value['type']) {
				...
				case 'sorter' :
					...
					//  assuming all sync'd, now get the correct naming for each block
					foreach( $sortlists as $key => $sortlist ) {
						foreach($sortlist as $k => $v) {
							$sortlist[$k] = $temp[$k];
						}
				...
				case 'sliderui' :
					...
					!isset($value['min']) 	? $s_min  	= '0' 			: $s_min  = $value['min'];
					!isset($value['max']) 	? $s_max  	= $s_min + 1 	: $s_max  = $value['max'];
					!isset($value['step']) 	? $s_step  	= '1' 			: $s_step = $value['step'];
