...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">'.__("Add New Slide", THEMETEXTDOMAIN).'</a></div>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">'.__("Upload", THEMETEXTDOMAIN).'</span>';

			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.__("Remove", THEMETEXTDOMAIN).'</span>';
		}
		else
		{
			$output .= '<p class="upload-notice"><i>'.__("Upgrade your version of WordPress for full media support.", THEMETEXTDOMAIN).'</i></p>';
		}
